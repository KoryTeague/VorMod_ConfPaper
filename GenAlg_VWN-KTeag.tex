...
The approximate Gaussian distribution $\rho^G$ is then standardized to $\rho^S = \rho^S\left(x,\, y\right), x \in [0,\, X], y \in [0,\, Y]$, which has a standard normal distribution.  The final log-normal distribution, $\rho = \rho\left(x,\, y\right), x \in [0,\, X], y \in [0,\, Y]$, is determined by assigning location and scale parameters $\mu$ and $\sigma$:
...
\sum_{s \in \mathcal{S}} c_s \; z_s - \nonumber \\
& \hspace{0.4in} \alpha \sum_{\omega \in \Omega} p^{(\omega)} \left( \sum_{m \in \mathcal{M}} \sum_{s \in \mathcal{S}} \delta_{ms}^{(\omega)} \; u_{ms}^{(\omega)} \right) \label{eq:P2}\\
& \text{subject to:}  \nonumber \\
& \hspace{0.4in} \sum_{s \in \mathcal{S}} \delta_{ms}^{(\omega)} \; u_{ms}^{(\omega)} \leq d_m,\, \forall m \in \mathcal{M},\, \forall \omega \in \Omega \label{eq:P2C1}\\
& \hspace{0.4in} \sum_{m \in \mathcal{M}} \delta_{ms}^{(\omega)} \leq r_s \; z_s,\, \forall s \in \mathcal{S},\, \forall \omega \in \Omega \label{eq:P2C2}\\
...
\hline 
Number of Scenarios ($|\Omega|$) & $\{5,\, 10,\, \ldots,\, 50\}$ \iffalse 25 \fi \\ 
\hline 
...
\begin{subfigure}{.45\textwidth}
	\centering
	\includegraphics[width=1\linewidth]{Figures/VOSGA_SpeedupRatio_AbsComp_alpha20}
	\caption{Speedup Ratio and Run Time}
	\label{fig:AlgSpeedupRunTime}
\end{subfigure}
\hspace{0.5cm}
\begin{subfigure}{.45\textwidth}
	\centering
	\includegraphics[width=1\linewidth]{Figures/ComparisonCombined}
	\caption{Absolute CPU Run Time}
...
\begin{figure}
	...
	\caption{Comparison of demand satisfaction with 50 new scenarios of 200 SP demand points.}
