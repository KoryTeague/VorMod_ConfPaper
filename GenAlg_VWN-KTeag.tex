...
We consider a geographical area of width $X$ m and length $Y$ m that contains a set $\mathcal{S} = \left\{1,\, 2,\, \ldots,\, S\right\}$ of base stations (BSs).  The rate capacity of BS $s \in \mathcal{S}$ is denoted by $r_s$, its cost is denoted by $c_s$, and its broadcasting range is denoted by $b_s$.
...
One major obstacle with solving Problem 1 is that stochastic equations are difficult to solve with commonly available tools\iffalse typical computational libraries\fi.  In order to be practical, Problem 1 may be reformulated such that it does not contain any stochastic variables.
...
We ran our simulations on an Intel Core i7-4790K 4.00 GHz CPU with 16 GB of DDR3 RAM.  We used CPLEX \cite{Cplex} to solve the DEP optimization problems and we used MATLAB to simulate the genetic algorithm and to generate the demand field and stochastic data (i.e., $\rho\left(x,\, y\right)$ and $u_{ms}^{\{\omega\}}$).  Average values for the performance of the genetic algorithm are provided from 50 independent runs using the identical data set except for the set of initial individuals.
